...
#generate an array of peak positions that shifts from 950 to 1200
#within a time array
x0_1_shift = spectral_shift(950, 1200, time)

#generate an array of peak positions that shifts from 1300 to 1200
#within a time array
x0_2_shift = spectral_shift(1300, 1200, time)
...
#generates a matrix that contains a gaussian at each time with a shift
#in peak position
species_2_matrix = gaussian_shift(nm, a2, x0_2_shift, sigma_2)
...
#spetral shift
data_matrix = data_1_matrix_shift + data_2_matrix_shift
...
output[0, 1:] = time
output[1:, 0] = nm
outfile = np.savetxt(date + '_twogaussian_spectralshfit_together.txt', output, fmt='%.3e', delimiter='\t')
