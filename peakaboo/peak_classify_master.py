...
def classify(data_nm, index_df, height_df, fwhm_df):
    ...
    while classify_ok == 'N':
        ...
        if default_parameter == 'Y':
            threshold = 0.001
            corrected_output = data_grouping(index_df, height_df, fwhm_df, threshold)
            ...
            classify_ok = input('Are you satisfied with peak classification? Y/N ')
            
            #make sure only Y or N is entered
            if classify_ok != 'Y' or 'N':
                print ('Please enter Y or N only for "Are you satisfied with peak classification?"')
                classify_ok = input('Are you satisfied with peak classification? Y/N ')
            ...
            threshold = int(input('Threshold (smaller than absolute value of the max/min signal): '))
            corrected_output = data_grouping(index_df, height_df, fwhm_df, threshold)
            corrected_output.describe()
            peak_dict = cluster_classifier(index_df, corrected_output)
            print (str(len(peak_dict)) + ' peaks are classified.')
