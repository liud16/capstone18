...
signal = np.expand_dims(signal_array, 1)

#signal_list = np.ndarray.tolist(signal_array)
#signal_list_reshape = np.reshape(signal_list, (-1, 1))
...
component_1 = signal_array * X_prob[:,1]
plt.figure()
plt.plot(time, component_1)

#model = pomegranate.GeneralMixtureModel.from_samples([pomegranate.NormalDistribution, pomegranate.NormalDistribution], n_components=2, X=signal_array)
#predict_pom = model.predict_proba
#
#dist_pom = model.distributions
#print dist_pom